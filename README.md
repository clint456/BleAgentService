...
BLE Agent Service 是一个蓝牙低功耗（BLE）设备服务，基于 EdgeX Foundry v4.0 框架构建。经过全面重构优化，基本现已达到**生产就绪**状态，为物联网应用提供稳定可靠的蓝牙通信解决方案。

该服务作为手机 App 与 EdgeX 微服务之间的智能桥梁，不仅提供完整的蓝牙通信功能，还具备强大的数据代理能力，能够将 EdgeX 数据转换为用户友好的格式并重新发布到消息总线。

### 🏆 项目亮点

- **🏗️ 完善的架构** - 模块化设计，职责分离，高可维护性
- **🚀 生产就绪** - 完善的错误处理、监控和日志记录
- **📈 高性能** - 优化的并发处理和资源管理
...
- **🔗 智能桥接** - 手机 App 与 EdgeX 微服务之间的无缝连接
- **📱 移动控制** - 通过手机 App 实现对 EdgeX 设备的远程监控和控制
- **🔄 数据代理** - 智能数据转换，将 EdgeX 数据重新发布供其他设备使用
...
详细重构信息请参考：[代码重构总结](REFACTORING_SUMMARY.md)
...
- **MQTT Broker**: Mosquitto 或其他 MQTT 代理

### 网络要求
- **局域网连接**: 用于连接 MQTT Broker
- **串口权限**: 对串口设备的读写权限
- **防火墙配置**: 开放服务端口 59995
