...
func SendJSONOverUART(port *SerialPort, jsonData []byte) error {
	// 将 JSON 数据序列化为字节
	...
	for _, packet := range packets {
		...
		readResponse(uart, packet.Index)
		// 模拟蓝牙模块的发送间隔（根据实际模块调整）
		time.Sleep(1 * time.Millisecond)
	}

	fmt.Println("✅️ All packets sent and verified.")
	return nil
}
